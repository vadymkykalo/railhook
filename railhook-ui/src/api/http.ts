...
/** Retry only when the API can't have rotated the cookie: a 500, 504 or timeout may have, and a reuse revokes every session. */
...
class HttpClient {
    ...
    this.client.interceptors.response.use(
      ...
      async (error: AxiosError) => {
        ...
        if (
          ...
          // A refused sign-out is a session already over; refreshing would revive it.
  ...
  /** Every waiter settles with the shared refresh; a resolve-only queue left failures pending forever. */
  ...
  /** Each refresh rotates the shared cookie; two tabs refreshing at once got everyone signed out. */
  ...
  private async runRefresh(): Promise<string> {
    try {
      ...
      // Rate limits, restarts and dropped connections don't end a session.
