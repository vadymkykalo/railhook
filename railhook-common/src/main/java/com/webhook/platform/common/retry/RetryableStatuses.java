 ...
 * No fallback: every row has a spec via a column default, so an unparseable spec is a bug.
 *
 * <p>Grammar: comma-separated terms, each {@code 429}, {@code 500-599}, {@code 5xx}, or
...
public final class RetryableStatuses {
    ...
    /**
     * The hard-coded set from before this was configurable, so leaving the field alone changes
     * nothing. {@code SchemaRetryLadderDefaultsTest} pins it against the Flyway defaults.
     */
    ...
    private static Range parseTerm(String term, String field, int position) {
        ...
        // Before the range split, or "2xx-3xx" would be read as a range.
    ...
    /** {@code <100} or {@code >599} is legal and selects nothing, like excluding everything. */
    ...
    private record Range(int from, int to) {
        Range {
            if (to < from) {
                // So BitSet.set(from, to + 1) is a no-op rather than a throw.
                to = from - 1;
            }
        }
    }
