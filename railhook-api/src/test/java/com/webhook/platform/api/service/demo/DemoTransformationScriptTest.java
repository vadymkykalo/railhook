...
// The script is the first thing a demo visitor runs; nothing else in the build would notice it throwing.
class DemoTransformationScriptTest {
    ...
    @Test
    void itReshapesTheItemsIntoLinesAndSetsWhatItAdvertises() throws Exception {
        ...
        assertThat(out.get("review").asText()).isEqualTo("manual");
        assertThat(out.get("placed_on").asText()).isEqualTo("2026-09-20");
        assertThat(outcome.headers()).containsEntry("X-Order-Value", "101.48");
        assertThat(outcome.cancelled()).isFalse();
    }

    @Test
    void theLinesAndTheReviewFlagFollowTheOrder() throws Exception {
        JsonNode small = payloadOf(run(order("39.98", item("WIDGET", 2, "19.99"))));
        assertThat(small.get("lines")).hasSize(1);
        assertThat(small.has("review")).isFalse();
