...
public class TunnelRegistry {
    ...
    // Keyed on the socket: a CLI that reconnected fast already has a new socket under the slug.
    ...
    /** Null if not connected or timed out; {@link #DISCONNECTED} if the socket closed mid-request. */
    ...
    // Only the socket the request went down may answer it.
    ...
    // Unregisters before closing, so the slug stops resolving at once.
