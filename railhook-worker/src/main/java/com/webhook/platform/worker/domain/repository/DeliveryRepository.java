...
public interface DeliveryRepository extends JpaRepository<Delivery, UUID> {
    ...
    // Not before its rung: a failed Attempt hands the row back PENDING with next_retry_at set,
    // and a second copy of the dispatch message (Kafka redelivery, an outbox re-publish) matching
    // on status alone took it at once — an Attempt the ladder had not reached, spending a rung.
    // A due row is the RetryGovernor's to hand out; a fresh one has no next_retry_at.
    // :now comes from the application, as findPendingRetryIds' does: next_retry_at is a timestamp
    // without time zone, and the database's now() depends on the session's zone.
    @Modifying
