...
- **An MCP method the server does not implement is answered as a JSON-RPC error**, not an HTTP 500.
  claude.ai probes `server/discover`; each probe was a 500 and counted towards the API's error-rate
  alert.
...
- **Railhook is listed on SaaSHub**, and the site footer and the README carry its badge. The footer's
  image is served from the site itself, so a visitor's browser makes no request to SaaSHub.
...
- **The docs keep their language switch on a phone**, where Starlight had moved it into the menu
  drawer, and every docs page links back to the site ("Back to site" / «На сайт»).
- A platform-admin audit test could pass its wait on rows from other tests and then fail; it now
  waits for the suspension's own rows.
