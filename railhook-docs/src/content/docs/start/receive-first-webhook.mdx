...
Railhook receives webhooks from a provider, verifies them and forwards them to your services. You need a project ID and an API key; the dashboard does the same under **Incoming**.

<Aside type="tip">
To see what a provider sends before connecting it, use the [webhook tester](https://railhook.io/tester): a URL that shows every request it receives, no account needed.
</Aside>
...
2. **Give the provider the ingress URL**
   ...
   <Aside type="caution">
   The token alone identifies the source, so treat the URL like a credential. Verification stops anyone who learns it from injecting events.
   </Aside>
