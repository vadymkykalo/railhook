---
title: Kubernetes
description: Встановіть Railhook у Kubernetes за допомогою Helm-чарта, спрямувавши його на власні PostgreSQL, Kafka та Redis.
---
...
<Aside type="danger">
Збережіть резервні копії `encryption-key` і `encryption-salt` і ніколи не змінюйте сіль на місці. Вона бере участь у виведенні ключа, тож зі зміненою сіллю кожен збережений секрет ендпоінта розшифровується в сміття.
</Aside>
...
| Тема | Поведінка |
|---|---|
| Міграції | Виконуються в поді API під час запуску. Репліки, що стартують разом, чекають на advisory lock PostgreSQL, тож кожна міграція застосовується один раз. Под воркера нового релізу чекає на них до 15 хвилин, перш ніж запуститися, і його startup probe дозволяє це очікування |
...
<CardGrid>
  <LinkCard title="Конфігурація" description="Змінні середовища, що стоять за значеннями чарта." href="/docs/uk/self-hosting/configuration/" />
