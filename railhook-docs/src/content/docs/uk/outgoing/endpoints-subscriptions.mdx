...
<Aside type="tip">
Щоб перевірити, чи ендпоінт відповідає, ще до реального трафіку, скористайтеся `POST /api/v1/projects/{projectId}/endpoints/{id}/test`. Він повідомляє, чи вдався запит, HTTP-статус і затримку. Повні формати запитів і відповідей — у [довіднику API](/docs/uk/api-reference/).
</Aside>
