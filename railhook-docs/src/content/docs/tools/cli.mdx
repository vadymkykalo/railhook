...
3. **Open a tunnel**
   ...
   The CLI prints a public URL and forwards every request that reaches it to `http://localhost:3000`, then returns your response. The URL is a base: a path after it is kept, so `https://<host>/tunnel/<slug>/webhooks/stripe` reaches `http://localhost:3000/webhooks/stripe`. Use it as an endpoint while you develop. The body reaches your app byte for byte, as the provider sent it, so your own signature check passes for form posts and binary bodies too, and your response goes back the same way. If the connection drops, the CLI reconnects to the same URL; a request already sent to your machine when it dropped is answered `502` straight away rather than after the 30-second timeout. Press Ctrl+C to close it.
...
<CardGrid>
  ...
  <LinkCard title="MCP server" description="Let an AI agent inspect deliveries and replay them." href="/docs/tools/mcp/" />
</CardGrid>
