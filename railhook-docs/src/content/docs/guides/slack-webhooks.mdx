...
description: Answer Slack's Events API within three seconds. Railhook verifies X-Slack-Signature, handles the url_verification challenge, and forwards events to your service with retries and replay.
---
import { Steps, Aside } from '@astrojs/starlight/components';

Slack sends Events API events to a Railhook source. The source verifies the signature, stores each event as it arrived and forwards it to your service with retries. Railhook answers Slack straight away, so a slow handler no longer gets your event subscriptions disabled.
...
A retry window of about five minutes does not cover a deploy that goes wrong. With Railhook in front:
...
Each replayed event goes to every destination of the source as a new forward:
