...
`monitoring/` in the repository contains Grafana, Prometheus, Alertmanager, Loki and Promtail, node-exporter, cAdvisor and blackbox-exporter, preconfigured for Railhook.

<Steps>
1. Add a Grafana password to `.env`. There is no default, and Grafana refuses to start without one:

   ```bash
   echo "GRAFANA_ADMIN_PASSWORD=$(openssl rand -base64 24)" >> .env
   ```

2. Set a receiver for alerts: `ALERTMANAGER_EMAIL_TO` (sent through your `SMTP_*` relay), `ALERTMANAGER_SLACK_WEBHOOK_URL`, `ALERTMANAGER_WEBHOOK_URL`, or `ALERTMANAGER_TELEGRAM_BOT_TOKEN` and `ALERTMANAGER_TELEGRAM_CHAT_ID`. With none set, alerts show only in Grafana.
3. Start the stack: `./railhook monitoring up` on a server installed with install.sh, `make monitoring-up` in a clone.
</Steps>
...
| Prometheus | Inside the stack only | Keeps `PROMETHEUS_RETENTION`, default `15d` |
| Alertmanager | Inside the stack only | `CRITICAL` alerts route faster than warnings; a firing critical alert suppresses the matching warning |
| Loki | Inside the stack only | Keeps logs for `LOKI_RETENTION_PERIOD`, default `168h` (7 days) |

[Monitoring](/docs/self-hosting/monitoring/) covers serving Grafana on its own domain and the host, container, uptime and log alerts.
...
| Metric | Reading it |
|---|---|
| `events_ingested_total` | Events accepted, tagged `direction` (`outgoing` from `/api/v1/events`, `incoming` from `/ingress`) |
| `deliveries_total` | Deliveries created by fan-out. Divided by events, the average subscriptions per event. |
...
| `webhook_dlq_depth`, `incoming_forward_dlq_depth` | Failed deliveries and forwards waiting for a person |
| `delivery_escalated_to_dlq_total` | Deliveries moved to Failed deliveries by the 96-hour hard cap rather than by running out of retries. Non-zero means something was degraded for days. |
...
`DlqActionableBacklog` and `IncomingForwardDlqActionableBacklog` are the ones that need a person: they clear when someone retries or purges the failed deliveries or forwards. `CircuitBreakerDegraded` is the odd one out among the breaker alerts: it means the breaker is **not** working.
