...
A project created before these rules were added automatically has none. Add them from the project's **PII masking** page, which offers the defaults while a project has no rules.

To check a rule before you rely on it, preview it against a sample payload on the project's **PII masking** page, or with the preview request in the [API reference](/docs/api-reference/).
