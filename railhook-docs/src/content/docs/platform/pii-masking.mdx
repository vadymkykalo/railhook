...
PII masking rules redact values in a payload when it is shown to a person: support staff, or anyone you send a debug link to. Rules are set per project.
...
Anyone with a shared debug link can open it without signing in, so masking is what makes it safe to send one outside your organization.
