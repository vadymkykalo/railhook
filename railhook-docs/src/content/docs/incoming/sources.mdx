...
| `429` | The source's rate limit is spent, with `Retry-After: 1`. Also returned, without `Retry-After`, when the organization cannot accept more webhooks right now; a resend of a webhook already stored is still answered `202`. |
