...
Every delivery is signed with the endpoint's secret. Verify the signature before you act on a request, so nobody who learns your endpoint URL can send you events.

## Two signature schemes

A delivery can carry two independent signatures over the same body. The endpoint's `signatureScheme` picks which:
...
Verify either one. Narrow the endpoint to one scheme only once nothing reads the other.
...
Read every `v1` in the header, not only the first.
...
These follow the published Standard Webhooks spec, so any library for it can verify them. Three differences from `X-Signature`:
...
The API returns `standardWebhooksSecret` next to `secret`, at creation and at rotation. The SDKs also accept a raw secret and use it as-is, so passing the wrong one fails as an invalid signature, not as a format error.
...
3. Deploy the new secret to the receiver inside that window. After it closes, only the new secret signs.
