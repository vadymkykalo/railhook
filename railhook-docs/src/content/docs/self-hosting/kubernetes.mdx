...
The Helm chart runs the API, the worker and the dashboard. You supply PostgreSQL, Kafka and Redis.
...
2. **Write a values file**
   ...
   With `app.baseUrl` empty, the chart takes the public URL from the first ingress host. Set it if Railhook is reached by another name.

3. **Install the chart**

   The chart is on GHCR for every release; its version is the release number without the `v`:
...
<Aside type="danger">
Back up `encryption-key` and `encryption-salt`, and never change the salt in place. It feeds key derivation, so a changed salt makes every stored endpoint secret decrypt to garbage.
</Aside>
...
| Topic | Behaviour |
|---|---|
| Migrations | Run inside the API pod at startup. Replicas starting together wait on a PostgreSQL advisory lock, so each migration is applied once. A worker pod from a new release waits up to 15 minutes for them before it starts, and its startup probe allows that wait |
