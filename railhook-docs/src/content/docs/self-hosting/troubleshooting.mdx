...
description: The problems self-hosted installations actually hit, what causes each one, and how to fix it.
---
...
| Symptom | Cause | Fix |
|---|---|---|
| `Port 80 is already in use` | Another service holds the port | Stop it, or install with `--port 8080`. With `--domain`, both 80 and 443 must be free |
...
| Source ingress URLs or tunnel URLs show `localhost:8080` | `TUNNEL_INGRESS_BASE_URL` is unset | Set it to the public URL |
...
<Aside type="tip">
In a restored or recovering installation, a burst of `delivery already claimed or not PENDING` messages in the worker log is expected. Stale Kafka messages are being declined, which is the intended behaviour.
</Aside>

## Still stuck

Open an issue at [github.com/vadymkykalo/railhook/issues](https://github.com/vadymkykalo/railhook/issues) with the output of `./railhook doctor` and the relevant log lines. Remove any secrets first.

## Next steps

<CardGrid>
  <LinkCard title="Configuration" description="What each setting decides." href="/docs/self-hosting/configuration/" />
  <LinkCard title="Monitoring" description="Catch problems before users report them." href="/docs/self-hosting/monitoring/" />
</CardGrid>
