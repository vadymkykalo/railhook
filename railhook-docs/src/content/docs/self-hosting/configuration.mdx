...
Most defaults are fine. These are development defaults that become wrong once the instance is reachable.
...
`ONBOARDING_EMAILS_ENABLED` sends two mails to new accounts: a welcome once the address is verified, and one nudge two days later if the account's organization has still sent and received no events. Each goes at most once per account, replies go to `EMAIL_SUPPORT_ADDRESS`, and nothing is sent unless `EMAIL_ENABLED` is on as well. Accounts that existed before the upgrade that added the setting never receive either.

Leave it off on your own instance. Both mails are written in the first person by Railhook's author for Railhook Cloud, and your users did not sign up with him.

## The blog

`BLOG_ENABLED` serves `/blog`, its posts and its feed. The image carries them only because railhook.io runs the same image as your instance: they are Railhook's own articles, not your site's.

It is off by default, and only railhook.io turns it on. Off, `/blog` and everything under it answer 404, the header and footer carry no Blog link, and `/sitemap.xml` lists no posts. The UI container reads it at startup, so changing it takes `docker compose up -d ui`, not a rebuild. On Kubernetes the setting is `ui.publicBlog`.
