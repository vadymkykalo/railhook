...
Both methods run the same `docker-compose.yml`. The installer also generates secrets and checks the configuration before starting anything.
...
<Tabs syncKey="install">
  <TabItem label="Installer">
    ...
    1. **Run the installer**
       ...
       It checks Docker, Compose, memory, disk and the port, then writes `~/railhook`: a `docker-compose.yml` pinned to the latest release, a `.env` with generated secrets, and a `railhook` helper script. It starts the stack and waits for the API. The first start runs the migrations and creates the Kafka topics, so it can take a few minutes.
    ...
    Any other argument goes to Docker Compose, so `./railhook ps` or `./railhook up -d` work.
  </TabItem>
